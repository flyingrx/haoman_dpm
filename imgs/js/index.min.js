...
function fillHistoryData(i) {
    ...
    if (i.data && 0 != i.data.length) {console.log(i.data);
        ...
        d += '<div class="avatar" ' + (e[l].avatar ? 'style="background-image: url(' + e[l].avatar + '?x-oss-process=style/avatar)"' : "") + ' avatar="' + e[l].avatar +' id="' + e[l].id + '" nickname="' + e[l].nickname + '" uid="' + e[l].from_user + '"></div>';
        ...
        d += '</div>';
            ...
            if (1 ==a||e[l].from_user==n) {
                ...
                if(e[l].from_user==n){
                    ...
                    d +='<span class="aDel" onclick="del(this,'+e[l].id+')" data-id="'+e[l].id+'">删除y</span>';
...
$.fn.wall = function () {
    ...
    function p(ispersonnal) {
        $.post(go_play_reward_seat, null, function (i) {
            if (0 == i.error) {
                s.guest_list = i.guest_list, s.item_list = i.item_list;console.log('i');console.log(s.item_list);
                var t = $(".fl-template>.fl-template-guest").html(), e = $("#ds-step-3").find(".fl-slide-content")[0];
                e = $(e), e.empty();

                for (var n = "", a = 0; a < s.guest_list.length; a++) {
                    var d = s.guest_list[a];
                    var dpic = rootimg + d.pic;
                    var detail = JSON.stringify(d.customer).replace(/\"/g,"'");
                    var l = t.replace("##name##", '第'+d.id+'桌').replace("##id##", d.id).replace("http://", dpic).replace("##role##", d.ds_times).replace("##detail##",detail);
                    d.id == s.ds_data.guest_id && (l = l.replace('"fl-ds-guest"', '"fl-ds-guest selected"')), a % 6 == 0 && ("" != n && (n += "</div>"), n += '<div class="fl-slide-panel">'), n += l
                }
                n += "</div>", e.html(n), $.TouchSlide({
                    slideCell: "#ds-step-3",
                    ...
                    s.ds_data.guest_id = $(this).attr("data-id"), $(".fl-ds-guest.selected").removeClass("selected"), $(this).addClass("selected"),$("input[name=guest_id]").val($(this).attr("data-id"));
                    s.ds_data.detail = $(this).attr("data-detail");
                });
                if(ispersonnal) $("#ds-next-btn").trigger('click');
            }else{
                $.tip('未获取到打赏对象、打赏礼物')
            }
        }, "json")
    ...
    /*function f(i) {
        for (var t = 0; t < s.guest_list.length; t++)if ('seat'+s.guest_list[t].id == i)return s.guest_list[t];
        return null
    }*/
    ...
    return $(this).find(".send_btn").click(function () {
        ...
        return "" == $.trim(i) ? void $.tip("信息为空") : (a.find(".msg_input_box").val(""), void $.post(go_sendmsg, {content: i}, function (i) {
            console.log(i.code), 1 == i.code ? (a.find(".showing_face").removeClass("showing_face"), a.find(".showing_menu").removeClass("showing_menu"), a.find(".msg_list_ctx").css("padding-bottom", "4rem"), $.tip(i.data), a.loadmsg()) : $.tip("信息发送失败...")
        }, "JSON"))
    }),
     a.find(".msg_input_menu .photo_file").upload_image(function (i) {
...
// debugger;
        ...
        $("#ds-step-opera-0").show().siblings().hide();
        $(".temp_ds_panel").hide()
        a.show();
        //点击选礼物 n:1 工作人员和节目 2 整桌 3用户
        function selectGift(){
            a.find("#ds-next-btn").unbind("click").click(function () {
                ...
                if (s.ds_data.guest_id > 0||x) {
                    if(x==s.ds_data.guest_id){
                        var i = e(s.ds_data.guest_id);
                        if(i.name)a.find(".ds-guest-name").text(i.name);
                    }
                    ...
                    $.TouchSlide({
                        ...
                        var num = $("#ds-num").val();
                        var total = (Number(i.price)*num).toFixed(2);
                        null != i && $(".ds-price").text(total), $("#ds-field-says").val($(this).attr("data-says"))
                    });
                    $("#ds-num").change(function () {
                        var num = $(this).val();
                        var p = $(".fl-ds-item.selected small").html();
                        var price = p?p.slice(1):0;
                        var total = (Number(price)*num).toFixed(2);
                        $(".ds-price").text(total);
                    })
                    $("#ds-step-2").show().siblings().hide();
                    $("#ds-step-opera-2").show().siblings().hide();
        ...
        selectGift()
        //工作人员和节目打赏
        a.find("#ds-staff").unbind('click').click(function () {
            a.find("#ds-step-1").show();
            $("#ds-step-opera-1").show();
            $("#ds-step-opera-0").hide();
            t();
        ...
        //用户或卡座打赏
        a.find("#ds-customer").unbind('click').click(function () {
            a.find("#ds-step-3").show();
            p();
// debugger;
            $('#ds-step-opera-3').show().siblings().hide();

            a.find("#ds-back-btn").unbind("click").click(function () {
                $("#ds-step-3").show(), $("#ds-step-opera-3").show(), $("#ds-step-2").hide(), $("#ds-step-opera-2").hide()
            });
            selectGift();

            a.find("#ds-table").unbind("click").click(function () {
                $("#ds-next-btn").trigger('click');
                $("input[name=tp]").val(2);
                /*var x= $("input[name=guest_id]").val();
                if(x<0||x==''){
                    $(".fl-ds-guest.selected").removeClass("selected")
                ...
                if (s.ds_data.guest_id) {
                    var i = e(s.ds_data.guest_id);
                    a.find(".ds-guest-name").text('卡座:'+i.id);
                    var t = $(".fl-template>.fl-template-item").html();
                    d = $("#ds-step-2").find(".fl-slide-content").first().empty();
                    html = "";
                    for (var l = 0; l < s.item_list.length; l++) {
                    ...
                    d.html(html);
                    // a.find("#ds-step-1").hide();
                    a.find("#ds-step-2").show().siblings().hide();
                    $.TouchSlide({
                        slideCell: "#ds-step-2",
                        titCell: ".fl-slide-pager ul",
                        mainCell: ".fl-slide-content",
            ...
            a.find("#ds-individual").unbind("click").click(function () {
                if (s.ds_data.detail) {
                    // s.guest_list = i.guest_list, s.item_list = i.item_list;

                    var t = $(".fl-template>.fl-template-guest").html(), e = $("#ds-step-3").find(".fl-slide-content")[0];
                    e = $(e), e.empty();
                    var fans = JSON.parse(s.ds_data.detail.replace(/\'/g,'"'));
                    for (var n = "", a = 0; a < fans.length; a++) {
                        var d = fans[a];
                        var dpic = rootimg + d.avatar;
                        var l = t.replace("##name##", d.nickname).replace("##id##", d.id).replace("http://", dpic).replace("##role##", d.ds_times).replace("##fuser##", d.from_user);
                        d.id == s.ds_data.guest_id && (l = l.replace('"fl-ds-guest"', '"fl-ds-guest selected"')), a % 6 == 0 && ("" != n && (n += "</div>"), n += '<div class="fl-slide-panel">'), n += l
                    }
                    n += "</div>", e.html(n), $.TouchSlide({
                        slideCell: "#ds-step-3",
                        ...
                        mainCell: ".fl-slide-content",
                        autoPage: !0
                    }), $(".fl-ds-guest").unbind('click').click(function () {
                        s.ds_data.fans_id = $(this).attr("data-id");
                        $(".fl-ds-guest.selected").removeClass("selected");
                        $(this).addClass("selected"),$("input[name=guest_id]").val($(this).attr("data-fuser"));
                        var nm = $(this).find('.fl-ds-guest-name').html();
                        $(".ds-guest-name").text(nm);
                    })
                    $("input[name=tp]").val(3);
                    var p = $("#ds-step-opera-1").show().siblings().hide();
                }else{
                    $.tip('未获取到打赏对象、打赏礼物')
                }
            })
        ...
        //点击个人头像时选择 打赏 ，对方同意后 出现私聊
        $("#personal_gift").click(function (e) {
            $(".wall_menu_ds").trigger('click');
            p(1);
            /*setTimeout(function () {
                $("#ds-next-btn").trigger('click');
            },1000)*/
    ...
    //}),
        ...
        if (t) {
           ...
           $(".bp_img_field img").attr("src", "../addons/haoman_dpm/imgs/zp.png"), $(".bp_img_field #upload").attr("accept", "image/*"), $(".bp_ctx textarea").val("")
        } else {
            var e = "小电影上墙";
            $(".mzh_modal_alert_title #title").html(e);
            $(".bp_img_field video").hide()
            $(".bp_img_field img").show()
            $(".bp_img_field img").attr("src", "../addons/haoman_dpm/imgs/video.png"), $(".bp_img_field #upload").attr("accept", "video/*"), $("input[name=type]").val("4"), $(".bp_ctx textarea").val("请大家看小电影")
        }
            $(".content video").hide();
            $(".bp_panel").show();
        $(".bp_tc_item").click(function () {
        ...
        a.find(".msg_input").removeClass("showing_menu").toggleClass("showing_face").hasClass("showing_face") ? a.find(".msg_list_ctx").css("padding-bottom", "16rem") : a.find(".msg_list_ctx").css("padding-bottom", "4rem")
        ...
        if (!$(this).parent().hasClass("mine")) {
            $(".userBox").fadeIn(), $(".userBox").find(".userBg").attr("src", $(this).attr("avatar")), $(".userBox").find(".send_msg").click(function () {
                location.href=go_talk+'&sid='+i.attr("uid");console.log(location.href)
            ...
            $(".userBox").find(".black_half_hour").click(function () {
                ...
                t && (t = !1, 0 == is_opened ? $.tip("大屏幕尚未开启~") : window.location.href = go_express_love +'&'+ i.attr("uid"))
            })
        }
    }), $(this).on("click", ".msg_box .admin_del_msg", function () {
        $.getJSON("/app/index.php?i=" + uniacid + "&c=entry&m=0&do=delmsg", {tid: [$(this).attr("val")]}, function (i) {
            location.reload()
        })
        ...
        $(this).next().click()
    }),$(this).on('click',".msg_box .avatar",function (e) {
        // var cr = e.target.getBoundingClientRect();
        // var db = e.target.getClientRects();
        var x = $(this).offset();
        $("#ava_tap").show();
        $("#ava_tap").offset({top:x.top,left:x.left-100});

        var uid = $(this).attr('uid');
        $("input[name=guest_id]").val(uid);
        $("input[name=tp]").val(3);
        $(".ds-guest-name").text($(this).attr('nickname'));
        e.stopPropagation();
        // console.log(cr);console.log();
        // console.log(db);
        /*var ava_tap = document.getElementById("ava_tap");
        console.log(cr);
        ava_tap.offsetLeft=cr.left;
        ava_tap.offsetTop=cr.top;
        ava_tap.classList.add('show');*/
        // $("#ava_tap").css({'offsetLeft':,'offsetTop':cr.top}).show();
        // var ul = document.createElement('ul');
        // ul.setAttribute('class',"ava_tap");
...
var rich = {
    ...
    dsShow:function(dt,bool){
        ...
        if(bool)
            dt.giftId = 'cake';
        var className,src,giftName,countTime,giftNum;
        giftNum = dt.gift_num;
        ...
        html += '<div class="dsObj">'+giftName+'</div><div class="ani-ds"> X <span id="ct_num"></span></div>';
//'+giftNum+'
        if(dt.say!=null)
        ...
        //数字连续变化
        var ct_num = document.getElementById('ct_num');
        if(ct_num){
            countUp(ct_num,giftNum,1,1000,0);
        }
