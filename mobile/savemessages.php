...
if (empty($message)) {
	$data = array('success' => 100, 'msg' => "留言不能为空");
...
$insert = array('uniacid' => $_W['uniacid'], 'avatar' => $avatar, 'nickname' => $nickname, 'from_user' => $from_user, 'word' => $message, 'wordimg' => $picture, 'rid' => $rid, 'status' => $status, 'is_back' => $fans['is_back'], 'is_xy' => $is_xys, 'is_bp' => 0, 'type' => 0, 'gift' => 0, 'createtime' => time());
$temp = pdo_insert('haoman_dpm_messages', $insert);
if ($temp == false) {
	$data = array('success' => 100, 'msg' => "上墙失败，请从新发送");
} else {
	$data = array('success' => 1, 'msg' => "发言成功");
}
echo json_encode($data);