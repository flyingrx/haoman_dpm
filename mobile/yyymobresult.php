...
if (empty($pici)) {
	$data = array('flag' => 11, 'msg' => "获取不到您的批次信息，请刷新后重试");
	echo json_encode($data);
	die;
}
if ($reply['pici'] == 1 && $reply['status'] == 0) {
	$data = array('flag' => 11, 'msg' => "活动还没开始，获取不到排名信息");
	echo json_encode($data);
	die;
}
if ($reply['status'] == 1) {
	$data = array('flag' => 11, 'msg' => "活动还未结束，请结束后再查看排名");
	echo json_encode($data);
	die;
}
$newPici = array();
...
if ($reply['pici'] != 1 && $pici != $reply['pici']) {
	...
	$data = array('flag' => 1, 'msg' => "您上轮排名第" . $order . "名");
} else {
	$pici = $reply['pici'];
	$data = pdo_fetchall("SELECT from_user FROM " . tablename('haoman_dpm_yyyuser') . " WHERE rid = :rid AND uniacid = :uniacid AND pici = :pici  AND point != 0 ORDER BY point DESC,endtime ASC", array(':rid' => $rid, ':uniacid' => $uniacid, ':pici' => $pici));
	$order = 1;
	...
	$data = array('flag' => 1, 'msg' => "您本轮排名第" . $order . "名");
}
echo json_encode($data);