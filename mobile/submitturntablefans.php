...
for ($i = 0; $i < $data; $i++) {
	...
	$insert = array('rid' => $rid, 'uniacid' => $_W['uniacid'], 'turntable' => 3, 'from_user' => $fans['from_user'], 'avatar' => $_GPC[$i . '_FansHead'], 'nickname' => $_GPC[$i . '_FansNickName'], 'awardname' => $prize['prizename'], 'awardsimg' => $prize['awardsimg'], 'prizetype' => $prize['ptype'], 'credit' => $prize['credit'], 'prize' => $prize['sort'], 'titleid' => $_GPC[$i . '_AwardId'], 'createtime' => time(), 'status' => 1, 'probalilty' => 1);
...
if ($stem) {
	$result = array('ResultType' => 1, 'msg' => $data);
	$this->message($result);
	die;
} else {
	$result = array('ResultType' => 0, 'msg' => "提交失败，请稍等在试！");
	$this->message($result);
	die;
}