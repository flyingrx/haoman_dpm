...
$fans = pdo_fetch("select id,from_user,is_back,avatar,nickname,totalnum,fanshbnum from " . tablename('haoman_dpm_fans') . " where from_user = :from_user ", array(':from_user' => $uid));

$hbaward = pdo_fetch("select id,credit from " . tablename('haoman_dpm_hb_award') . " where from_user = :from_user and prize=:prize", array(':from_user' => $uid,':prize'=>$hbid));
    ...
    if(empty($hb)){
       ...
       if($hb['hbnum']-$hb['usesnum']<1){
           ...
           if($credit>0){
               ...
               if($hbs['hbtype']==1){
                   pdo_update('haoman_dpm_fans', array('fanshbnum' => $fans['fanshbnum'] + $credit), array('id' => $fans['id']));
